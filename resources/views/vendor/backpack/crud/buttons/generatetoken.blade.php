@if ($crud->hasAccess('generatetoken'))
<a
    href="javascript:void(0)"
    onclick="updateEntry(this)"
    data-route="{{ url($crud->route.'/generateToken/'.$entry->getKey())}}"
    class="btn btn-sm btn-link"
    data-button-type="patch"
    ><i class="las la-clone"></i> Generate Token</a
>
@endif
...
<script>
    if (typeof updateEntry != "function") {
        ...
        function updateEntry(button) {
            ...
            swal({
                title: "Do you want to generate a token for this user ?",
                icon: "warning",
                buttons: ["Cancel", "Yes"],
                dangerMode: true,
            }).then((value) => {
